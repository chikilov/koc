...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$("#btnUpload").click(function () {
				if ( $("#upload_image")[0].files.length )
				{
					...
					$.ajax({
						url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/uploadImage/0",
						processData: false,
						contentType: false,
						dataType:"json",
						type:"POST",
			...
			$("#btnPreview").click(function () {
				...
				window.open("<?php echo URLBASE; ?>index.php/pages/notice/listnotice/preview/<?php $subnavi?>/", "previewpopup", "width=1200, height=600");
			});
			...
			$("#btnInsert").click(function () {
				...
				if ( $("#notice_target").val() == "" || $("#notice_target").val() == null )
				{
					alert("대상");
					$("#notice_target").focus();
					return;
				...
				if ( $("#thumbnail")[0].files.length > 0 )
				{
					...
					$.ajax({
						url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/uploadImage/1",
						processData: false,
						contentType: false,
						dataType:"json",
						type:"POST",
						...
						success:function(data)
						{
							...
							if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
							{
								insertNotice( obj.arrResult.file_name );
							}
							else
							{
								alert(obj.resultText);
				...
				if ( $("#thumbnail:visible").length > 0 )
				{
					...
					$.ajax({
						...
						success:function(data)
						{
							...
							if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
							{
								updateNotice( obj.arrResult.file_name );
							}
							else
							{
								alert(obj.resultText);
			...
			{
				$.ajax({
					url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/requestNoticeListModi",
					type:"POST",
					dataType:"json",
					async:false,
					...
					success:function(data)
					{
						...
						if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
						{
							$("#notice_title").val(obj.arrResult[0].notice_title);
							...
							$("#notice_target").val(obj.arrResult[0].notice_target);
		...
		function insertNotice( file_name )
		{
			$.ajax({
				url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/requestNoticeListInsert",
				type:"POST",
				dataType:"json",
				async:false,
				data: {"data":"\{\"notice_title\":\"" + $("#notice_title").val() + "\", \"thumbnail\":\"" + file_name + "\", \"start_date\":\"" + $("#start_date").val() + " " + $("#start_hour > option:selected").val() + ":" + $("#start_min > option:selected").val() + ":00\",\"end_date\":\"" + $("#end_date").val() + " " + $("#end_hour > option:selected").val() + ":" + $("#end_min > option:selected").val() + ":00\",\"notice_target\":\"" + $("#notice_target").val() + "\",\"content_type\":\"" + $("#content_type > option:selected").val() + "\",\"content_text\":\"" + $("#content_text").val().replace(/\n/gi, "<br />") + "\", \"content_image\":\"" + $("#content_image").val() + "\", \"content_link\":\"" + $("#content_link").val() + "\"\}"},
				success:function(data)
				{
					var obj = eval(data);
					if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
					{
						alert("등록되었습니다.");
						window.location = "<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/view/0";
					}
					else
					{
						alert(obj.resultText);
		...
		function updateNotice( file_name )
		{
			if ( $("#thumbnail:visible").length > 0 )
			{
				file_name = file_name;
			...
			$.ajax({
				url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/requestNoticeListUpdate",
				type:"POST",
				dataType:"json",
				async:false,
		...
		{
			window.open("<?php echo URLBASE; ?>static/upload/notice/thumbnail/" + url, "previewpopup", "width=1200, height=600");
		}

		function showimage(url)
		{
			window.open("<?php echo URLBASE; ?>static/upload/notice/content/" + url, "previewpopup", "width=1200, height=600");
		}
	</script>
</head>
<body>
				...
				<div class="board_view_st alignL">
					<table>
						...
						<tbody>
							<tr>
								<th>제목</th>
								<td colspan="3"><input type="text" id="notice_title" name="notice_title" style="width: 96%;" /></td>
							</tr>
							<tr>
								<th>썸네일</th>
								<td colspan="3">
...
?>
								...
								<th>대상</th>
								<td>
									<select name="notice_target" id="notice_target">
...
<?php
	...
	{
?>
				<div class="btnPosR"><a href="javascript:void(0);" id="btnUpdate" class="btn_action"><span>수정</span></a>&nbsp;<a href="javascript:void(0);" id="btnPreview" class="btn_action"><span>미리보기</span></a>&nbsp;<a href="<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/view/0" class="btn_action"><span>취소</span></a></div>
<?php
	}
	else
	{
?>
				<div class="btnPosR"><a href="javascript:void(0);" id="btnInsert" class="btn_action"><span>등록</span></a>&nbsp;<a href="javascript:void(0);" id="btnPreview" class="btn_action"><span>미리보기</span></a>&nbsp;<a href="<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/view/0" class="btn_action"><span>취소</span></a></div>
<?php
	}
?>
			</div>

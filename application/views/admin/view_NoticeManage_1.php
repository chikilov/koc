	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#btnOrderConf").click(function () {
				...
				$.ajax({
					url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/requestEventListOrderChange",
					type:"POST",
					dataType:"json",
					async:false,
					data: {"data":strParam},
		...
		function notiarray( mIdx, mOrder_no, mEvent_title, mStart_date, mEnd_date )
		{
			this.idx = mIdx;
			this.order_no = mOrder_no;
			this.event_title = mEvent_title;
			this.start_date = mStart_date;
			this.end_date = mEnd_date;
		}

		function eventListPage(page)
		{
			...
			var strPaging = "<a href=\"javascript:eventListPage(" + prev + ");\" class=\"prev\" title=\"이전\"></a>";
			strPaging += "<ul>";
			for ( var i = prev; i < next; i++ )
			{
				if ( i == page )
				{
					strPaging += "<li><strong><a href=\"javascript:eventListPage(" + i + ");\">" + i + "</a></strong></li>";
				}
				else
				{
					strPaging += "<li><a href=\"javascript:eventListPage(" + i + ");\">" + i + "</a></li>";
				}
			}
			strPaging += "</ul>";
			if ( next <= totPage )
			{
				strPaging += "<a href=\"javascript:eventListPage(" + next + ");\" class=\"next\" title=\"다음\"></a>";
			}
			...
			if ( notiArray.length > 0 )
			{
				for ( var i = ( ( page - 1 ) * pageSize ); i < notiArray.length && i < ( page * pageSize ); i++ )
				{
					...
					strNotiInfo += "<td>" + notiArray[i].event_title + "</td>";
					strNotiInfo += "<td class=\"alignC\"><a href=\"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/form/1/" + notiArray[i].idx + "\" class=\"btn_basic\">수정</a>&nbsp;&nbsp;";
					strNotiInfo += "<a href=\"javascript:delNotiList('" + notiArray[i].idx + "');\" class=\"btn_basic\">삭제</a></td>";
					strNotiInfo += "</tr>";
				}
				$("#eventTbody").html(strNotiInfo);
		...
		function delNotiList( idx )
		{
			$.ajax({
				url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/del/1/" + idx,
				type:"POST",
				dataType:"json",
				async:false,
				data: null,
		...
		function getEventList()
		{
			notiArray = new Array();
			$.ajax({
				url:"<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/requestEventList",
				type:"POST",
				dataType:"json",
				async:false,
				data: null,
				success:function(data)
				{
					...
					if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
					{
						...
						{
							notiArray.push( new notiarray(obj.arrResult[i].idx, obj.arrResult[i].order_no, obj.arrResult[i].event_title, obj.arrResult[i].start_date, obj.arrResult[i].end_date) );
						}
...
<div id="wrap">
	...
	<div id="contents" class="contents">
		...
		<div class="section">
			...
			<div class="paging">
				<div class="btnPosR"><a href="<?php echo URLBASE; ?>index.php/pages/admin/noticemanage/form/1" class="btn_action"><span>등록</span></a>&nbsp;<a href="javascript:void(0);" id="btnOrder" class="btn_action"><span>순서변경</span></a><a style="display:none;" href="javascript:void(0);" id="btnOrderConf" class="btn_action"><span>확인</span></a></div>
			</div>
			<!-- //Paging -->
			<!--공지 리스트-->
			<!-- <div class="container">

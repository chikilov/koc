...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$(document).on("change", "#evt_paytype", function () {
				if ( $("#evt_paytype > option:selected").val() == "DIS" )
				{
					$("#dis_unit").html("%");
				}
				else
			...
			$("#btnInsert").click(function () {
				...
				if ( $("#evt_paytype").val() == "" || $("#evt_paytype").val() == null )
				{
					alert("할인방법");
					$("#evt_paytype").focus();
					return;
			...
			$("#btnConf1").click(function () {
				$("#creatAlert2").hide();
				$.ajax({
					url:"<?php echo URLBASE; ?>index.php/pages/admin/eventmanage/requestDisEventInsert",
					type:"POST",
					dataType:"json",
					async:false,
					data: {"data":"\{\"start_date\":\"" + $("#start_date").val() + " " + $("#start_hour > option:selected").val() + ":" + $("#start_min > option:selected").val() + ":00\",\"end_date\":\"" + $("#end_date").val() + " " + $("#end_hour > option:selected").val() + ":" + $("#end_min > option:selected").val() + ":00\",\"evt_category\":\"" + $("#evt_category > option:selected").val() + "\",\"evt_target\":\"" + $("#evt_target > option:selected").val() + "\",\"evt_paytype\":\"" + $("#evt_paytype > option:selected").val() + "\",\"evt_value\":\"" + $("#evt_value").val() + "\",\"evt_reason\":\"" + $("#evt_reason").val() + "\"\}"},
					success:function(data)
					{
						var obj = eval(data);
						if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
						{
							...
							$("#evt_paytype > option:selected").prop("selected", false)
							$("#evt_value").val("");
							$("#evt_reason").val("");
			...
			$(document).on("click", "a[name=btnStop]", function () {
				...
				{
					$.ajax({
						url:"<?php echo URLBASE; ?>index.php/pages/admin/eventmanage/requestDisEventStop",
						type:"POST",
						dataType:"json",
						async:false,
		...
		function eventarray( mIdx, mStart_date, mEnd_date, mEvt_category, mEvt_target, mEvt_paytype, mEvt_value, mIs_valid, mReg_date )
		{
			this.idx = mIdx;
		...
		function eventPage(page)
		{
			...
			if ( evtArray.length > 0 )
			{
				for ( var i = ( ( page - 1 ) * pageSize ); i < evtArray.length && i < ( page * pageSize ); i++ )
				{
					...
					strEventInfo += "<td>" + evtArray[i].evt_target + "(" + evtArray[i].evt_category + "), ";
					if ( evtArray[i].evt_paytype == "DIS" )
					{
						strEventInfo += "할인, " + evtArray[i].evt_value + "%";
					}
					else if ( evtArray[i].evt_paytype == "CASH_POINTS" )
					{
						strEventInfo += "수정, " + evtArray[i].evt_value + "개";
					}
					else if ( evtArray[i].evt_paytype == "GAME_POINTS" )
					{
						strEventInfo += "골드, " + evtArray[i].evt_value + "개";
					}
					else if ( evtArray[i].evt_paytype == "FRIENDSHIP_POINTS" )
					{
						strEventInfo += "우정포인트, " + evtArray[i].evt_value + "개";
					}
					else if ( evtArray[i].evt_paytype == "IAP" )
					{
						strEventInfo += "인앱결제, " + evtArray[i].evt_value + "원";
					}
					strEventInfo += "</td>";
		...
		function getEventList()
		{
			evtArray = new Array();
			$.ajax({
				url:"<?php echo URLBASE; ?>index.php/pages/admin/eventmanage/requestDisEventList",
				type:"POST",
				dataType:"json",
				async:false,
				data: null,
				success:function(data)
				{
					...
					if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
					{
						if ( obj.arrResult.length > 0 )
						{
							for ( var i = 0; i < obj.arrResult.length; i++ )
							{
								evtArray.push( new eventarray(obj.arrResult[i].idx, obj.arrResult[i].start_date, obj.arrResult[i].end_date, obj.arrResult[i].evt_category, obj.arrResult[i].evt_target, obj.arrResult[i].evt_paytype, obj.arrResult[i].evt_value, obj.arrResult[i].is_valid, obj.arrResult[i].reg_date ) );
							}
						}
...
?>
							...
							<tr>
								...
								<th>할인방법</th>
								<td><select name="evt_paytype" id="evt_paytype">
									<option value="">= 선택 =</option>
									<option value="DIS">할인</option>
									<option value="CASH_POINTS">수정</option>
									<option value="GAME_POINTS">골드</option>
									<option value="FRIENDSHIP_POINTS">우정포인트</option>
								</select>&nbsp;<input type="text" id="evt_value" name="evt_value" style="width:50px; text-align: right;" value="" /><span id="dis_unit"></span></td>
							</tr>
							<tr>
	...
	<div class="alertPop" style="display:none; top:300px; left:50%; margin-left:-35px" id="creatAlert2">
		<p>적용 하시겠습니까?</p>
		<div class="btnArea alignC">
			<a href="javascript:void(0);" id="btnConf2" class="btn_action sm addUiBtn">확인</a>
			<a href="javascript:void(0);" class="btn_basic sm alertClose">취소</a>
		</div>
	</div>

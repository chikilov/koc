	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#btn_search").click(function () {
				...
				{
					$.ajax({
						url:"<?php echo URLBASE; ?>index.php/pages/admin/accountdetail/pilotInfo",
						type:"POST",
						dataType:"json",
						async:false,
						...
						success:function(data)
						{
							...
							if ( obj.resultCd == "<?php echo MY_Controller::STATUS_API_OK?>" )
							{
								...
								{
									inventoryArray.push( new inventoryarray( obj.inventoryInfo[i].idx, obj.inventoryInfo[i].itemname, obj.inventoryInfo[i].board, obj.inventoryInfo[i].reg_date ) );
								}
								...
								if ( itemobj.length > 0 )
								{
									for ( var i = 0; i < itemobj.length && i < pageSize; i++ )
									{
										...
										{
											strInventoryInfo += "<td>" + itemobj[i].board + "</td>";
										}
										...
										{
											strInventoryInfo += "<td>" + itemobj[i].reg_date + "</td>";
										}
										strInventoryInfo += "<td>?</td>";
										strInventoryInfo += "<td>?</td>";
									}
			...
			$("#btn_conf").click(function () {
				...
				if ( arrayCheckedIndex.length > 0 )
				{
					...
					for ( var i = 0; i < arrayCheckedIndex.length; i++ )
					{
						...
						{
							$.ajax({
								url:"<?php echo URLBASE; ?>index.php/pages/admin/accountdetail/pilotDelete",
								type:"POST",
								dataType:"json",
						...
						{
							if ( confirm( "삭제할 경우 배정된 팀정보도 모두 삭제됩니다.\n그래도 실행하시겠습니까?" ) )
							{
								isConfirmed = true;
								$.ajax({
									url:"<?php echo URLBASE; ?>index.php/pages/admin/accountdetail/pilotDelete",
		...
		function inventoryarray( mIdx, mItemname, mBoard, mReg_date )
		{
			this.idx = mIdx;
					...
					<table>
						...
						<thead>
							<tr>
								...
								<th class="alignC bb2">탑승</th>
								<th class="alignC bb2">구매일자</th>
						...
						<tbody id="itemTbody">
							<tr>
								<td colspan="7" class="alignC">데이터가 없습니다.</td>
							</tr>
						</tbody>
					</table>
				</div>
